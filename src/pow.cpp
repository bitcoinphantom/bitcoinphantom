...
{
    const int64_t nTargetTimespan = 1 * 30;  // quick work-around for the old global timespan
    unsigned int nStakeTargetSpacing = 0.1;		// 0.1 seconds POS block spacing
    const int64_t nTargetSpacingWorkMax = 2 * nStakeTargetSpacing; // 10 seconds
...
unsigned int LwmaCalculateNextWorkRequired(const CBlockIndex* pindexLast, int algo, const Consensus::Params& params)
{
    ...
    //  Some coins took out a variable, and need to change the 2*30*30 here:
    //  if (block.GetBlockTime() > nAdjustedTime + 2 * 30 * 30)
...
unsigned int GetAlgoWeight(int algo)
{
    ...
    {
        case ALGO_GROESTL:
            return (unsigned int)(0.002 * 100000);
        case ALGO_BLAKE:
            return (unsigned int)(0.00008 * 100000);
        case ALGO_X17:
            return (unsigned int)(2 * 100000);
        case ALGO_LYRA2RE:
            return (unsigned int)(2 * 100000);
        case ALGO_SCRYPT:
            return (unsigned int)(0.7 * 100000);
        default: // Lowest
            printf("GetAlgoWeight(): can't find algo %d", algo);
            return (unsigned int)(0.00002 * 100000);
    }
}
